...
if [[ $YEAR != "year" ]]
  then
    ...
    if [[ -z $TEAMS ]]
    then
      INSERT_MAJOR_RESULT=$($PSQL "INSERT INTO games(year,round,winner_id,opponent_id,winner_goals,opponent_goals) VALUES('$YEAR','$ROUND','$WINNER_ID','$OPPONENT_ID','$WINNER_GOALS','$OPPONENT_GOALS')")
      if [[ $INSERT_MAJOR_RESULT == "INSERT 0 1" ]]
      then
        echo Inserted into games, $GAMES
      fi
    fi
  fi
done